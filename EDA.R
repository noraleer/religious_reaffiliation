library(foreign)
dat <- read.spss("/Users/gregorymatthews/Dropbox/CDSC/ryanwong/FinalCleanedData.sav")
dat <- as.data.frame(dat)

names(dat)[1] <- "ID"
dat_long <- dat %>% select(ID, CatholicT1:UnknownIncomeRatioT4) %>% 
        pivot_longer(
                cols = -ID,
...
write.csv(dat_long , file = "/Users/gregorymatthews/Dropbox/CDSC/ryanwong/FinalCleanedData_long.csv", row.names = FALSE)
